...
### might have to mutate data below to filter out alive ppl but get the gist
```{r}

ggplot(data = cirrhosis, aes (Drug, Stage, fill = N_Days))  + geom_tile() + scale_fill_distiller(palette = "RdPu") 
...
cirrhosis %>%
  ggplot() +
  geom_mosaic(aes( x = product(Edema), fill = Drug))
#ggplot(data = cirrhosis) + geom_mosaic(aes(x = Drug , fill = Status))

```
Looking at the dist. of stages
```{r}

cirrhosis %>%
      ggplot(aes(x = Stage)) + geom_bar() # need color

```

```{r}
cirrhosis %>%
      ggplot(aes(x = Cholesterol, y = Bilirubin)) + geom_line()
