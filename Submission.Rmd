...
![Primary biliary cholangitis (<https://www.mayoclinic.org/diseases-conditions/primary-biliary-cholangitis/symptoms-causes/syc-20376874>)](diagram.jpeg)

## Why is the data important to us?


We are very interested in the intersection of medicine and machine learning. To us, clinical research is engaging and interesting to learn about. It is also special because of the impact it can have on the healthcare industry. There is so much potential for advancement of medicine using AI, and we are passionate about this. The 'cirrhosis' data set stood out to us because the data set included many diverse attributes. A limitation was that this dataset had only 418 cases. This reflects a very probable issue when using clinical data. Thus, our goal is to use our Statistical DS skillet to approach this problem despite its limitations.
...
The dataset is produced from a clinical study by Mayo Clinic run from 1974-1984. The final 'Status' of each patient was observed in 1986. 'Status' was either Dead, Censored, or Censored due to liver transplant. A link to the original study is in our references. 
...
Our goal is to explore the data through our research questions while demonstrating our understandings of including regression, classification and clustering. Let us first clear some definitions and differences overall.

[Unsupervised vs Supervised learning]{.underline}
...
[Regression vs Clustering]{.underline}

-   **Regression Learning(logistic)**:Logistic regression is a process of modeling the probability of a dichotomous outcome given an input variable.
...
Exploring how liver-related conditions (ascites, hepatomegaly, and spiders) affect patient survival provides insights into the severity of these complications in PBC progression. 
...
Survival Curves are commonly used in medical research to measure the ability of a patient to survive. There was a survivial curve in the original study. Out of curiosity, we attempted one here. The combination of of Ascites, Hepatomegaly, and Spiders shows many overlapping curves with often large variation. The labels cut off here, but this is more of a visual exploration. 



# Part I: Classification Algorithm for the Status of the Patient using CART

To address the first sub-goal of this project, we will be exploring the prediction of the status of a patient at the end of the study. Our main objective through this is to gain a stronger understanding of what factors played in the role of the death of the patient. To do this, we will be using Decision Trees. 
...
We ca see the pruned tree has cut out some leaf nodes. This would help in avoiding overfitting the model. As a reminder, Overfitting is when a model learns the detail and noise in the training data to the extent that it negatively impacts the performance of the model on new data.
...
We can see that for this data, surprisingly, the tree based approach had better results. This is an uncommon observation. We believe this was because of the sparcity of the data and how the rows N/A values prevented over fitting with the small data size.
...
We were able to visualize the results of the k-means algorithm. We created a new color palette and plotted the dendrogram.
...
```
From the table, we see the mean values for each variable belonging to each group.Group 5, for instance, has the largest mean value for Cholestorol.


From the graph, we can see that the breakdown of the different stages in our clusters. It's important to note that the amount of patients with each stage was not equal in that data and that is shown here. However, there are new observations to be made as our findings produced 5 clusters. 


# Discussion

To conclude, in this report, we were able to explore Biliary Cirrhosis and Treatments through various angles. We were able to build two different kinds of models: decision tree and logistic regression models. Bilirubin, Age, and Prothrombin were strong predictors to predict the status of the patient. From our clustering algorithm, we were able to create 5 clusters for our data and compared it with the stages of the patients.


Along with the analysis in the report project, we attempted to explore various models such as predicting the likelihood of a liver transplant and the number of days until an event. However, due to our data's and report's limitations, such as the limited data quantities, we were unable to get concrete solutions. s 
...
Fedesoriano. (2021, August 2). Cirrhosis prediction dataset. Kaggle. https://www.kaggle.com/datasets/fedesoriano/cirrhosis-prediction-dataset/data 


Dickson, et. al. (n.d.). Prognosis in primary biliary cirrhosis: Model for decision making. Prognosis in Primary Biliary Cirrhosis: Model for Decision Making. https://faculty.washington.edu/abansal/ShortCourse_DynamicDecisionMaking/Dickson1989_MayoPBCOriginalArticle.pdf 

Penn Medicine. (n.d.). Primary Biliary Cirrhosis. Pennmedicine.org. https://www.pennmedicine.org/for-patients-and-visitors/patient-information/conditions-treated-a-to-z/primary-biliary-cirrhosis#:~:text=All%20of%20the%20bile%20ducts,This%20is%20called%20biliary%20cirrhosis. 
...
professional, C. C. medical. (n.d.). Primary biliary cholangitis: What it is, symptoms, treatment. Cleveland Clinic. https://my.clevelandclinic.org/health/diseases/17715-primary-biliary-cholangitis-pbc 
